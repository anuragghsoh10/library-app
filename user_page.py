...
from request_book import request_book_window
from return_book import return_book_window
...
def user_page(username, role):
    ...
    def show_page(page_func, *args):
        clear_content()
        page_func(content, *args)
    ...
    def show_dashboard():
        ...
        cards=[
            ...
            ("📩 Request Book", lambda: show_page(request_book_window, username)),
            ("🔄 Return Book", lambda: show_page(return_book_window, username))
        ]
    ...
    btn1 = tk.Button(sidebar,text="🔍 View Books",**btn_style,
                    command=lambda:[activate(btn1),show_page(view_books_window)])
    btn1.pack(pady=5)

    btn2 = tk.Button(sidebar,text="📩 Request Book",**btn_style,
                    command=lambda:[activate(btn2),show_page(request_book_window, username)])
    btn2.pack(pady=5)

    btn3 = tk.Button(sidebar,text="🔄 Return Book",**btn_style,
                    command=lambda:[activate(btn3),show_page(return_book_window, username)])
    btn3.pack(pady=5)
